...
var validator = FormValidation.formValidation(
    form,
    {
        fields: {
            ...
            'savings_cash_mutation_amount_view': {
                validators: {
                    notEmpty: {
...
$(document).ready(function(){
    ...
    $("#savings_cash_mutation_amount_view").change(function(){
        ...
        document.getElementById("savings_cash_mutation_amount_view").value  = toRp(savings_cash_mutation_amount);
        function_elements_add('savings_cash_mutation_amount', savings_cash_mutation_amount);
        calculate();
    });
    
    $("#savings_cash_mutation_amount_adm_view").change(function(){
        var savings_cash_mutation_amount_adm                                    = $("#savings_cash_mutation_amount_adm_view").val();
        document.getElementById("savings_cash_mutation_amount_adm").value       = savings_cash_mutation_amount_adm;
        document.getElementById("savings_cash_mutation_amount_adm_view").value  = toRp(savings_cash_mutation_amount_adm);
        function_elements_add('savings_cash_mutation_amount_adm', savings_cash_mutation_amount_adm);
        calculate();
    });
...
function function_elements_add(name, value){
    $.ajax({
        type: "POST",
        url : "{{route('savings-cash-mutation.elements-add')}}",
        data : {
            'name'      : name, 
            'value'     : value,
            '_token'    : '{{csrf_token()}}'
...
function calculate(){
    ...
    var savings_cash_mutation_amount_adm    = $("#savings_cash_mutation_amount_adm").val();
    var savings_account_last_balance        = $("#savings_account_last_balance").val();

    if(mutation_id == 1 || mutation_id == 3){
        var savings_cash_mutation_last_balance  = parseFloat(savings_account_last_balance) + parseFloat(savings_cash_mutation_amount);
    }else{
        var savings_cash_mutation_last_balance  = parseFloat(savings_account_last_balance) - parseFloat(savings_cash_mutation_amount);
    }

    document.getElementById("savings_cash_mutation_last_balance").value = savings_cash_mutation_last_balance;
    document.getElementById("savings_cash_mutation_last_balance_view").value = toRp(savings_cash_mutation_last_balance);
    function_elements_add('savings_cash_mutation_last_balance', savings_cash_mutation_last_balance);
    ...
    <div class="card">
        <div class="card-header">
            <div class="card-title m-0">
                <h3 class="fw-bolder m-0">{{ __('Form Tambah Mutasi Tabungan Tunai') }}</h3>
            </div>
            <a href="{{ route('savings-cash-mutation.index') }}" class="btn btn-light align-self-center">
        ...
        <div id="kt_savings_cash_mutation_view">
            <form id="kt_savings_cash_mutation_add_view_form" class="form" method="POST" action="{{ route('savings-cash-mutation.process-add') }}" enctype="multipart/form-data">
            @csrf
            @method('POST')
                    ...
                    <div class="row mb-6">
                        ...
                        <div class="col-lg-6">
                            ...
                            <div class="row mb-6">
                                ...
                                <div class="col-lg-8 fv-row">
                                    <input type="text" name="savings_cash_mutation_amount_view" id="savings_cash_mutation_amount_view" class="form-control form-control-lg form-control-solid" placeholder="Rupiah" value="{{ old('savings_cash_mutation_amount_view', number_format($sessiondata['savings_cash_mutation_amount'], 2) ?? '') }}" autocomplete="off"/>
                                    <input type="hidden" name="savings_cash_mutation_amount" id="savings_cash_mutation_amount" class="form-control form-control-lg form-control-solid" placeholder="Rupiah" value="{{ old('savings_cash_mutation_amount', $sessiondata['savings_cash_mutation_amount'] ?? '') }}" autocomplete="off"/>
                                </div>
                            </div>
                            <div class="row mb-6">
                            ...
                            <div class="row mb-6">
                                ...
                                <div class="col-lg-8 fv-row">
                                    <input type="text" name="savings_cash_mutation_last_balance_view" id="savings_cash_mutation_last_balance_view" class="form-control form-control-lg form-control-solid" placeholder="Saldo Baru" value="{{ old('savings_cash_mutation_last_balance_view', number_format($sessiondata['savings_cash_mutation_last_balance'], 2) ?? '') }}" autocomplete="off" readonly/>
                                    <input type="hidden" name="savings_cash_mutation_last_balance" id="savings_cash_mutation_last_balance" class="form-control form-control-lg form-control-solid" placeholder="Saldo Baru" value="{{ old('savings_cash_mutation_last_balance', $sessiondata['savings_cash_mutation_last_balance'] ?? '') }}" autocomplete="off"/>
                                </div>
                            </div>
                            <div class="row mb-6">
                                <label class="col-lg-4 col-form-label fw-bold fs-6 required">{{ __('Tanggal Mutasi') }}</label>
                                <div class="col-lg-8 fv-row">
                                    <input type="text" name="savings_cash_mutation_date" class="form-control form-control-lg form-control-solid" placeholder="Tanggal Mutasi" value="{{ date('d-m-Y') }}" autocomplete="off" readonly/>
                                </div>
                            </div>
                            <div class="row mb-6">
                                <label class="col-lg-4 col-form-label fw-bold fs-6  ">{{ __('Keterangan') }}</label>
                                <div class="col-lg-8 fv-row">
                                    <textarea id="savings_cash_mutation_remark" name="savings_cash_mutation_remark" class="form-control form-control form-control-solid" data-kt-autosize="true" placeholder="Keterangan" onchange="function_elements_add(this.name, this.value)">{{ old('savings_cash_mutation_remark', $sessiondata['savings_cash_mutation_remark'] ?? '') }}</textarea>
                                </div>
                            </div>
                        </div>
                    </div>
