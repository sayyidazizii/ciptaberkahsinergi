...
<x-base-layout>
    <div class="card">
        ...
        <form id="kt_filter_credits_payment_form" class="form" method="POST" action="{{ route('credits-payment-bank.filter') }}" enctype="multipart/form-data">
            @csrf
            @method('POST')
            <div id="kt_card_collapsible" class="collapse">
                ...
                <div class="card-footer d-flex justify-content-end py-6 px-9">
                    <a href="{{ route('credits-payment-bank.filter-reset') }}" class="btn btn-danger me-2" id="kt_filter_cancel">
                        {{__('Batal')}}
                    </a>
    ...
    <div class="card">
        <div class="card-header">
            <h3 class="card-title">Daftar Angsuran Via Bank</h3>
            <div class="card-toolbar">
                <a type="button" href="{{ route('credits-payment-bank.add') }}"  class="btn btn-sm btn-light-primary">
                    {!! theme()->getSvgIcon("icons/duotune/general/gen035.svg", "svg-icon-2x me-1") !!}
                    {{ __('Tambah Angsuran Via Bank Baru') }}
                </a>
            </div>
        </div>
        <div class="card-body pt-6">
            @include('content.AcctCreditsPaymentBank.List._table')
        </div>
    </div>
</x-base-layout>