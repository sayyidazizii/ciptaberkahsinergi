...
const form = document.getElementById('kt_credits_payment_bank_view_form');
...
const submitButton = document.getElementById('kt_credits_payment_bank_submit');
...
$(document).ready(function(){
    $('#open_modal_button').click(function(){
        $.ajax({
            type: "GET",
            url : "{{route('credits-payment-bank.modal-credits-account')}}",
...
function function_elements_add(name, value){
    $.ajax({
        type: "POST",
        url : "{{route('credits-payment-bank.elements-add')}}",
        data : {
            'name'      : name, 
            'value'     : value,
            '_token'    : '{{csrf_token()}}'
    ...
    <div class="card">
        <div class="card-header">
            <div class="card-title m-0">
                <h3 class="fw-bolder m-0">{{ __('Form Tambah Angsuran Via Bank') }}</h3>
            </div>
            <a href="{{ route('credits-payment-bank.index') }}" class="btn btn-light align-self-center">
                {!! theme()->getSvgIcon("icons/duotune/arrows/arr079.svg", "svg-icon-4 me-1") !!}
                {{ __('Kembali') }}
            </a>
        </div>

        <div id="kt_credits_payment_bank_view">
            <form id="kt_credits_payment_bank_view_form" class="form" method="POST" action="{{ route('credits-payment-bank.process-add') }}" enctype="multipart/form-data">
            @csrf
            @method('POST')
                <div class="card-body pt-6">
                    <div class="row mb-6">
                        <div class="col-lg-6">
                            <div class="row mb-6">
                                <b class="col-lg-12 fw-bold fs-3 text-primary">{{ __('Pinjaman & Bank') }}</b>
                            </div>
                            <div class="row mb-6">
                            ...
                            <div class="row mb-6">
                                ...
                                <div class="col-lg-8 fv-row">
                                    <select name="bank_account_id" id="bank_account_id" aria-label="{{ __('Bank Transfer') }}" data-control="select2" data-placeholder="{{ __('Pilih bank..') }}" data-allow-clear="true" class="form-select form-select-solid form-select-lg" onchange="function_elements_add(this.name, this.value)">
                ...
                <div class="card-footer d-flex justify-content-end py-6 px-9">
                    ...
                    <button type="submit" class="btn btn-primary" id="kt_credits_payment_bank_submit">
