...
function changeCity(name = null,value = null){
    ...
    $.ajax({
        ...
        data: {
            'city_id'   : city_id,
            'last_kecamatan_id'   : "{{old('kecamatan_id', $sessiondata['kecamatan_id'] ?? '')}}",
            '_token'        : '{{csrf_token()}}',
        },
...
function changeKecamatan(name = null,value = null){
    ...
    $.ajax({
        ...
        data: {
            ...
            'last_kelurahan_id' :  "{{old('kelurahan_id', $sessiondata['kelurahan_id'] ?? '')}}",
            '_token'        : '{{csrf_token()}}',
        },
        success: function(return_data){
                    ...
                    <div class="row mb-6">
                                ...
                                <div class="col-lg-12 fv-row">
                                    <select name="city_id" id="city_id" aria-label="{{ __('Kabupaten') }}" data-control="select2" data-placeholder="{{ __('Pilih Kabupaten..') }}" data-allow-clear="true" class="form-select form-select-solid form-select-lg select2-hidden-accessible" onchange="changeCity()">
                                        ...
                                        @foreach($akun as $key => $value)
                                            <option data-kt-flag="{{ $key }}" value="{{ $key }}" {{ $key === old('city_id', $sessiondata['city_id'] ?? '') ? 'selected' :'' }}>{{ $value }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col mb-6">
                                ...
                                <div class="col-lg-12 fv-row">
                                    <input name="dusun_name" id="dusun_name" type="text" class="form-control form-control-solid form-select-lg" placeholder="Masukan Nama Dusun" value="{{ old('dusun_name', $sessiondata['dusun_name'] ?? '') }}" onchange="function_elements_add(this.name, this.value)"/>
                                </div>
                            </div>
                        </div>
                    </div>
