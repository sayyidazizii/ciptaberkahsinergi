@section('scripts')
<script>
const form = document.getElementById('kt_mutation_add_view_form');
...
const submitButton = document.getElementById('kt_mutation_add_submit');
...
<x-base-layout>
    <div class="card mb-5 mb-xl-10">
        ...
        <div id="kt_mutation_add_view">
            <form id="kt_mutation_add_view_form" class="form" method="POST" action="{{ route('mutation.process-add') }}" enctype="multipart/form-data">
            @csrf
            @method('POST')
                <div class="card-body border-top p-9">
                    <div class="row mb-6">
                        ...
                        <div class="col-lg-8 fv-row">
                            <input type="text" name="mutation_code" class="form-control form-control-lg form-control-solid" placeholder="Kode Mutasi" value="{{ old('mutation_code', '' ?? '') }}" autocomplete="off"/>
                        </div>
                    </div>
                    <div class="row mb-6">
                        <label class="col-lg-4 col-form-label fw-bold fs-6 required">{{ __('Nama Mutasi') }}</label>
                        <div class="col-lg-8 fv-row">
                            <input type="text" name="mutation_name" class="form-control form-control-lg form-control-solid" placeholder="Nama Mutasi" value="{{ old('mutation_name', '' ?? '') }}" autocomplete="off"/>
                        </div>
                    </div>
                    <div class="row mb-6">
                        <label class="col-lg-4 col-form-label fw-bold fs-6 required">{{ __('Fungsi Mutasi') }}</label>
                        <div class="col-lg-8 fv-row">
                            <input type="text" name="mutation_function" class="form-control form-control-lg form-control-solid" placeholder="+/-" value="{{ old('mutation_function', '' ?? '') }}" autocomplete="off"/>
                        </div>
                    </div>
                    <div class="row mb-6">
                        ...
                        <div class="col-lg-8 fv-row">
                            <select name="mutation_status" id="mutation_status" aria-label="{{ __('Pilih Status Mutasi') }}" data-control="select2" data-placeholder="{{ __('Pilih status mutasi..') }}" data-allow-clear="true" class="form-select form-select-solid form-select-lg">
                                ...
                                @foreach($accountstatus as $key => $value)
                                    <option data-kt-flag="{{ $key }}" value="{{ $key }}" {{ $key === old('mutation_status', '' ?? '') ? 'selected' :'' }}>{{ $value }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                ...
                <div class="card-footer d-flex justify-content-end py-6 px-9">
                    ...
                    <button type="submit" class="btn btn-primary" id="kt_mutation_add_submit">
