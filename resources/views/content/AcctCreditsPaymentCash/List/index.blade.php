...
<x-base-layout>
    <div class="card">
        ...
        <form id="kt_filter_credits_payment_form" class="form" method="POST" action="{{ route('credits-payment-cash.filter') }}" enctype="multipart/form-data">
            @csrf
            @method('POST')
    ...
    <div class="card">
        <div class="card-header">
            <h3 class="card-title">Daftar Angsuran Tunai</h3>
            <div class="card-toolbar">
                <a type="button" href="{{ route('credits-payment-cash.add') }}"  class="btn btn-sm btn-light-primary">
                    {!! theme()->getSvgIcon("icons/duotune/general/gen035.svg", "svg-icon-2x me-1") !!}
                    {{ __('Tambah Angsuran Tunai Baru') }}
                </a>
            </div>
        </div>
        <div class="card-body pt-6">
            @include('content.AcctCreditsPaymentCash.List._table')
        </div>
    </div>
</x-base-layout>