...
<x-base-layout>
    <div class="card">
        ...
        <form id="kt_filter_savings_bank_mutation_form" class="form" method="POST" action="{{ route('savings-bank-mutation.filter') }}" enctype="multipart/form-data">
            @csrf
            @method('POST')
            <div id="kt_card_collapsible" class="collapse">
                ...
                <div class="card-footer d-flex justify-content-end py-6 px-9">
                    <a href="{{ route('savings-bank-mutation.filter-reset') }}" class="btn btn-danger me-2" id="kt_filter_cancel">
                        {{__('Batal')}}
                    </a>
    ...
    <div class="card">
        <div class="card-header">
            ...
            <div class="card-toolbar">
                <a type="button" href="{{ route('savings-bank-mutation.add') }}"  class="btn btn-sm btn-light-primary">
                    {!! theme()->getSvgIcon("icons/duotune/general/gen035.svg", "svg-icon-2x me-1") !!}
                    {{ __('Tambah Mutasi Tabungan Via Bank Baru') }}
                </a>
            </div>
        </div>
        <div class="card-body pt-6">
            @include('content.AcctSavingsBankMutation.List._table')
        </div>
    </div>
</x-base-layout>