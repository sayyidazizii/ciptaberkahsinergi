...
<x-base-layout>
    <div class="card">
        ...
        <form id="kt_filter_general-ledger_form" class="form" method="POST" action="{{ route('profit-loss-report-consolidation.filter') }}" enctype="multipart/form-data">
            @csrf
            @method('POST')
    ...
    <div class="card">
                ...
                <div class="col-lg-8">
                    <div class="table-responsive">
                        <table class="table table-sm table-rounded border gy-3 gs-3 show-border">
                            <thead>
                                <tr align="center">
                                    <th colspan="2" class="align-middle"><b>LAPORAN LABA RUGI KONSOLIDASI</b></th>
                                </tr>
                                <tr align="center">
                                            ...
                                            <?php
                                                foreach ($acctprofitlossreport_top as $key => $val) {
                                                    ...
                                                    if($val['report_type']	== 3){
                                                        $account_subtotal 	= AcctProfitLossReportConsolidationController::getAccountAmount($val['account_id'], $sessiondata['start_month_period'], $sessiondata['end_month_period'], $sessiondata['year_period'], $sessiondata['branch_id'],$sessiondata['profit_loss_report_type']);
        ...
        <div class="card-footer d-flex justify-content-end py-6 px-9">
            <a href="{{ route('profit-loss-report-consolidation.export') }}" class="btn btn-primary me-2">{{ __('Export Excel') }}</a>
            <a href="{{ route('profit-loss-report-consolidation.print') }}" class="btn btn-primary">{{ __('Export PDF') }}</a>
        </div>
    </div>
    <script>
