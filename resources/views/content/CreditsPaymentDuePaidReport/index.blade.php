    ...
    <div class="card mb-5 mb-xl-10">
        <div class="card-header border-0">
            <div class="card-title m-0">
                <h3 class="fw-bolder m-0">{{ __('Daftar Pinjaman Jatuh Tempo') }}</h3>
            </div>
        </div>

        <div id="kt_credits_payment_report_view">
            <form id="kt_credits_payment_report_view_form" class="form" method="POST" action="{{ route('credits-payment-due-report.viewport') }}" enctype="multipart/form-data">
            @csrf
            @method('POST')
