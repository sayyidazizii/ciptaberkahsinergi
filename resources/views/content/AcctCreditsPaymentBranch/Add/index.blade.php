...
const form = document.getElementById('kt_credits_payment_branch_view_form');
...
const submitButton = document.getElementById('kt_credits_payment_branch_submit');
...
$(document).ready(function(){
    $('#open_modal_button').click(function(){
        $.ajax({
            type: "GET",
            url : "{{route('credits-payment-branch.modal-credits-account')}}",
...
function function_elements_add(name, value){
    $.ajax({
        type: "POST",
        url : "{{route('credits-payment-branch.elements-add')}}",
        data : {
            'name'      : name, 
            'value'     : value,
            '_token'    : '{{csrf_token()}}'
    ...
    <div class="card">
        <div class="card-header">
            <div class="card-title m-0">
                <h3 class="fw-bolder m-0">{{ __('Form Tambah Angsuran Tunai (Antar Cabang)') }}</h3>
            </div>
            <a href="{{ route('credits-payment-branch.index') }}" class="btn btn-light align-self-center">
                {!! theme()->getSvgIcon("icons/duotune/arrows/arr079.svg", "svg-icon-4 me-1") !!}
                {{ __('Kembali') }}
            </a>
        </div>

        <div id="kt_credits_payment_branch_view">
            <form id="kt_credits_payment_branch_view_form" class="form" method="POST" action="{{ route('credits-payment-branch.process-add') }}" enctype="multipart/form-data">
            @csrf
            @method('POST')
                        ...
                        <div class="col-lg-6">
                            ...
                            <div class="row mb-6">
                                ...
                                <div class="col-lg-5 fv-row">
                                    ...
                                    <input type="hidden" name="credits_account_branch_id" class="form-control form-control-lg form-control-solid" placeholder="No Akad Pinjaman" value="{{ old('credits_account_branch_id', $acctcreditsaccount['branch_id'] ?? '') }}" autocomplete="off" readonly/>
                                </div>
                ...
                <div class="card-footer d-flex justify-content-end py-6 px-9">
                    ...
                    <button type="submit" class="btn btn-primary" id="kt_credits_payment_branch_submit">
