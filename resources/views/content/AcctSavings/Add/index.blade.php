...
const submitButton = document.getElementById('kt_savings_add_submit');
    ...
    <div class="card mb-5 mb-xl-10">
        <div class="card-header border-0">
            <div class="card-title m-0">
                <h3 class="fw-bolder m-0">{{ __('Form Tambah Kode Tabungan') }}</h3>
            </div>
        ...
        <div id="kt_savings_add_view">
            <form id="kt_savings_add_view_form" class="form" method="POST" action="{{ route('savings.process-add') }}" enctype="multipart/form-data">
            @csrf
            @method('POST')
                <div class="card-body border-top p-9">
                    <div class="row mb-6">
                        ...
                        <div class="col-lg-8 fv-row">
                            <input type="text" name="savings_code" class="form-control form-control-lg form-control-solid" placeholder="Kode Tabungan" value="{{ old('savings_code', '' ?? '') }}" autocomplete="off"/>
                        </div>
                    </div>
                    <div class="row mb-6">
                        <label class="col-lg-4 col-form-label fw-bold fs-6 required">{{ __('Nama Tabungan') }}</label>
                        <div class="col-lg-8 fv-row">
                            <input type="text" name="savings_name" class="form-control form-control-lg form-control-solid" placeholder="Nama Tabungan" value="{{ old('savings_name', '' ?? '') }}" autocomplete="off"/>
                        </div>
                    </div>
                    <div class="row mb-6">
                        ...
                        <div class="col-lg-8 fv-row">
                            <select name="account_id" id="account_id" aria-label="{{ __('Nomor Perkiraan Tabungan') }}" data-control="select2" data-placeholder="{{ __('Pilih nomor perkiraan..') }}" data-allow-clear="true" class="form-select form-select-solid form-select-lg">
                                ...
                                @foreach($acctaccount as $key => $value)
                                    <option data-kt-flag="{{ $value['account_id'] }}" value="{{ $value['account_id'] }}" {{ $value['account_id'] === old('account_id', '' ?? '') ? 'selected' :'' }}>{{ $value['full_account'] }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="col-lg-8 fv-row">
                            <select name="savings_profit_sharing" id="savings_profit_sharing" aria-label="{{ __('Pilih Status Tabungan') }}" data-control="select2" data-placeholder="{{ __('Pilih status tabungan..') }}" data-allow-clear="true" class="form-select form-select-solid form-select-lg">
                                ...
                                @foreach($savingsprofitsharing as $key => $value)
                                    <option data-kt-flag="{{ $key }}" value="{{ $key }}" {{ $key === old('savings_profit_sharing', '' ?? '') ? 'selected' :'' }}>{{ $value }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    <div class="row mb-6">
                        ...
                        <div class="col-lg-8 fv-row">
                            <select name="account_basil_id" id="account_basil_id" aria-label="{{ __('Nomor Perkiraan Bunga Tabungan') }}" data-control="select2" data-placeholder="{{ __('Pilih nomor perkiraan..') }}" data-allow-clear="true" class="form-select form-select-solid form-select-lg">
                                <option value="">{{ __('Pilih nomor perkiraan..') }}</option>
                                @foreach($acctaccount as $key => $value)
                                    <option data-kt-flag="{{ $value['account_id'] }}" value="{{ $value['account_id'] }}" {{ $value['account_id'] === old('account_basil_id', '' ?? '') ? 'selected' :'' }}>{{ $value['full_account'] }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    <div class="row mb-6">
                        <label class="col-lg-4 col-form-label fw-bold fs-6 required">{{ __('Bunga Tabungan') }}</label>
                        <div class="col-lg-8 fv-row">
                            <input type="text" name="savings_interest_rate" class="form-control form-control-lg form-control-solid" placeholder="%" value="{{ old('savings_interest_rate', '' ?? '') }}" autocomplete="off"/>
                        </div>
                    </div>
                    <div class="row mb-6">
