...
<x-base-layout>
    <div class="card">
        ...
        <form id="kt_filter_credits_account_form" class="form" method="POST" action="{{ route('credits-account-master.filter') }}" enctype="multipart/form-data">
            @csrf
            @method('POST')
            <div id="kt_card_collapsible" class="collapse">
                ...
                <div class="card-footer d-flex justify-content-end py-6 px-9">
                    <a href="{{ route('credits-account-master.filter-reset') }}" class="btn btn-danger me-2" id="kt_filter_cancel">
                        {{__('Batal')}}
                    </a>
    ...
    <div class="card">
        <div class="card-header">
            ...
            <div class="card-toolbar">
                <a type="button" href="{{ route('credits-account-master.export') }}"  class="btn btn-sm btn-light-primary">
                    <i class="bi bi-download fs-2"></i>
                    {{ __('Export Data Pinjaman') }}
                </a>
            </div>
        </div>
        <div class="card-body pt-6">
            @include('content.AcctCreditsAccountMaster.List._table')
        </div>
    </div>
</x-base-layout>