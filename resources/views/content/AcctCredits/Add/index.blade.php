@section('scripts')
<script>
const form = document.getElementById('kt_credits_add_view_form');
...
const submitButton = document.getElementById('kt_credits_add_submit');
...
<x-base-layout>
    <div class="card mb-5 mb-xl-10">
        ...
        <div id="kt_credits_add_view">
            <form id="kt_credits_add_view_form" class="form" method="POST" action="{{ route('credits.process-add') }}" enctype="multipart/form-data">
            @csrf
            @method('POST')
                <div class="card-body border-top p-9">
                    <div class="row mb-6">
                        ...
                        <div class="col-lg-8 fv-row">
                            <input type="text" name="credits_code" class="form-control form-control-lg form-control-solid" placeholder="Kode Pinjaman" value="{{ old('credits_code', '' ?? '') }}" autocomplete="off"/>
                        </div>
                    </div>
                    <div class="row mb-6">
                        <label class="col-lg-4 col-form-label fw-bold fs-6 required">{{ __('Nama Pinjaman') }}</label>
                        <div class="col-lg-8 fv-row">
                            <input type="text" name="credits_name" class="form-control form-control-lg form-control-solid" placeholder="Nama Pinjaman" value="{{ old('credits_name', '' ?? '') }}" autocomplete="off"/>
                        </div>
                    </div>
                    <div class="row mb-6">
                        ...
                        <div class="col-lg-8 fv-row">
                            <select name="receivable_account_id" id="receivable_account_id" aria-label="{{ __('Nomor Perkiraan Pinjaman') }}" data-control="select2" data-placeholder="{{ __('Pilih nomor perkiraan..') }}" data-allow-clear="true" class="form-select form-select-solid form-select-lg">
                                ...
                                @foreach($acctaccount as $key => $value)
                                    <option data-kt-flag="{{ $value['account_id'] }}" value="{{ $value['account_id'] }}" {{ $value['account_id'] === old('receivable_account_id', '' ?? '') ? 'selected' :'' }}>{{ $value['full_account'] }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    <div class="row mb-6">
                        ...
                        <div class="col-lg-8 fv-row">
                            <select name="income_account_id" id="income_account_id" aria-label="{{ __('Nomor Perkiraan Bunga Pinjaman') }}" data-control="select2" data-placeholder="{{ __('Pilih nomor perkiraan..') }}" data-allow-clear="true" class="form-select form-select-solid form-select-lg">
                                <option value="">{{ __('Pilih nomor perkiraan..') }}</option>
                                @foreach($acctaccount as $key => $value)
                                    <option data-kt-flag="{{ $value['account_id'] }}" value="{{ $value['account_id'] }}" {{ $value['account_id'] === old('income_account_id', '' ?? '') ? 'selected' :'' }}>{{ $value['full_account'] }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    <div class="row mb-6">
                        ...
                        <div class="col-lg-8 fv-row">
                            <input type="text" name="credits_fine" class="form-control form-control-lg form-control-solid" placeholder="%" value="{{ old('credits_fine', '' ?? '') }}" autocomplete="off"/>
                        </div>
                    </div>
                </div>
                <div class="card-footer d-flex justify-content-end py-6 px-9">
                    ...
                    <button type="submit" class="btn btn-primary" id="kt_credits_add_submit">
