@section('scripts')
<script>
const form = document.getElementById('kt_daily_bank_flow_report_view_form');
...
<x-base-layout>
    <div class="card">
        <form id="kt_daily_bank_flow_report_view_form" class="form" method="POST" action="{{ route('daily-bank-flow-report.print') }}" enctype="multipart/form-data">
        @csrf
        @method('POST')
            <div class="card-body pt-6">
                <div class="row mb-6">
                    ...
                    <div class="col-lg-6 fv-row">
                        <label class="col-form-label fw-bold fs-6">{{ __('Bank') }}</label>
                        <select name="account_id" id="account_id" data-control="select2" data-placeholder="{{ __('Pilih Bank') }}" data-allow-clear="true" class="form-select form-select-solid form-select-lg">
                            <option value="">{{ __('Pilih') }}</option>
                            @foreach($acctbank as $key => $value)
                                <option data-kt-flag="{{ $value['account_id'] }}" value="{{ $value['account_id'] }}" {{ $value['account_id'] == old('account_id', '' ?? '') ? 'selected' :'' }}>{{ $value['bank_account_name'] }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-lg-6 fv-row">
