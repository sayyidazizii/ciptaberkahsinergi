@section('scripts')
<script>
const form = document.getElementById('kt_savings_account_mutation_view_form');
...
$(document).ready(function(){
    $('#open_modal_button').click(function(){
        $.ajax({
            type: "GET",
            url : "{{route('savings-account-mutation.modal-savings-account')}}",
...
const submitButton = document.getElementById('kt_member_mutation_submit_print');
...
const submitButton2 = document.getElementById('kt_member_mutation_submit_preview');
...
function function_elements_add(name, value){
    $.ajax({
        type: "POST",
        url : "{{route('savings-account-mutation.elements-add')}}",
        data : {
            'name'      : name, 
            'value'     : value,
            '_token'    : '{{csrf_token()}}'
...
function searchDate(){
    ...
    $.ajax({
        type: "POST",
        url : "{{route('savings-account-mutation.filter')}}",
        data : {
...
<x-base-layout>
    <div class="card">
        <div class="card-header">
            <h3 class="card-title">Cetak Mutasi Buku</h3>
            <div class="card-toolbar">
            </div>
        </div>

        <div id="kt_savings_account_mutation_view">
            <form id="kt_savings_account_mutation_view_form" class="form" method="POST" action="{{ route('savings-account-mutation.print') }}" enctype="multipart/form-data">
            @csrf
            @method('POST')
                <div class="card-body pt-6">
                    <div class="row mb-6">
                        <div class="col-lg-6">
                            ...
                            <div class="row mb-6">
                                <label class="col-lg-4 col-form-label fw-bold fs-6">{{ __('No. Terakhir') }}</label>
                                <div class="col-lg-8 fv-row">
                                    <input type="text" name="savings_account_last_number" class="form-control form-control-lg form-control-solid" placeholder="No. Terakhir" value="{{ old('savings_account_last_number', $datases['savings_account_last_number'] ?? '') }}" autocomplete="off" onchange="function_elements_add(this.name, this.value)"/>
                                </div>
                            </div>
                        </div>
                        <div class="col-lg-6">
                            ...
                            <div class="row mb-6">
                                ...
                                <div class="col-lg-8">
                                    <a href="{{ route('savings-account-mutation.reset-filter') }}" class="btn btn-danger me-2" id="kt_member_mutation_reset"  name="kt_member_mutation_reset" value="batal">
                ...
                <div class="card-footer d-flex justify-content-end py-6 px-9">
                    <button type="submit" class="btn btn-primary me-2" id="kt_member_mutation_submit_preview">
                        @include('partials.general._button-indicator', ['label' => __('Preview')])
                    </button>
                    <button type="submit" class="btn btn-primary" id="kt_member_mutation_submit_print">
                        @include('partials.general._button-indicator', ['label' => __('Cetak')])
                    </button>
                </div>
            </form>
        </div>
