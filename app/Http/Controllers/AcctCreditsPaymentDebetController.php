...
{
    public function index(AcctCreditsPaymentDebetDataTable $dataTable)
    {
        session()->forget('data_creditspaymentdebetadd');
        $sessiondata = session()->get('filter_creditspaymentdebet');
    ...
    public function filter(Request $request){
        ...
        session()->put('filter_creditspaymentdebet', $sessiondata);

        return redirect('credits-payment-debet');
    }

    public function filterReset(){
        session()->forget('filter_creditspaymentdebet');
    ...
    {
        $sessiondata = session()->get('data_creditspaymentdebetadd');
        ...
        session()->put('data_creditspaymentdebetadd', $sessiondata);
    }
    ...
    {
        return $dataTable->render('content.AcctCreditsPaymentDebet.Add.AcctCreditsAccountModal.index');
    }
    ...
    public function selectAcctCreditsAccount($credits_account_id)
    {
        ...
        return redirect('credits-payment-debet/add');
    }

    public function modalAcctSavingsAccount(AcctSavingsAccountDataTable $dataTable)
    {
        return $dataTable->render('content.AcctCreditsPaymentDebet.Add.AcctSavingsAccountModal.index');
    }
    ...
    public function processAdd(Request $request)
    {
        ...
        if($acctsavingsaccount['savings_account_last_balance'] < $request->angsuran_total){
            ...
            return redirect('credits-payment-debet/add')->with($message);
        }
        ...
        try {
            ...
            $acctsavingsaccount_edit = AcctSavingsAccount::findOrFail($fields['savings_account_id']);
            $acctsavingsaccount_edit->savings_account_last_balance = $acctsavingsaccount['savings_account_last_balance'] - $data['credits_payment_amount'];
            $acctsavingsaccount_edit->save();

            $last_balance 	= ($acctsavingsaccount['savings_account_last_balance']) - $data['credits_payment_amount'];
            $mutasi_data 	=array(
            ...
            if($data['credits_payment_fine'] > 0){
                $last_balance_after_fine 	= $last_balance - $data['credits_payment_fine'];
                $mutasi_data 	=array(
                    "savings_account_id" 					=> $request->savings_account_id,
                    "savings_id" 							=> $acctsavingsaccount['savings_id'],
                    "member_id" 							=> $acctsavingsaccount['member_id'],
            ...
            $savingsaccount_id 			= AcctSavings::select('account_id')
            ->where('savings_id', $mutasi_data['savings_id'])
            ->first()
            ->account_id;

            $account_id_default_status 	= AcctAccount::select('account_default_status')
            ->where('account_id', $savingsaccount_id)
            ->first()
            ...
            ->where('credits_id', $acctcashpayment_last['credits_id'])
            ->first()
            ...
            if($data['credits_others_income'] > 0){
                $account_id_default_status 	= AcctAccount::select('account_default_status')
            ...
            DB::commit();
            $message = array(
                'pesan' => 'Angsuran Debet Tabungan berhasil ditambah',
                'alert' => 'success'
            );
        } catch (\Exception $e) {
            DB::rollback();
    ...
    public function printNote($credits_payment_id){
        ...
        <table cellspacing=\"0\" cellpadding=\"1\" border=\"0\">
            <tr>
                <td width=\"25%\"></td>
                <td width=\"75%\"><div style=\"text-align: left; font-size:14px\">BUKTI SETORAN ANGSURAN DEBET</div></td>
            </tr>
            <tr>
                <td width=\"25%\"></td>
        ...
        <table cellspacing=\"0\" cellpadding=\"1\" border=\"0\" width=\"100%\">
            ...
            <tr>
                <td width=\"20%\"><div style=\"text-align: left;\">No. Rek. Tabungan</div></td>
                <td width=\"50%\"><div style=\"text-align: left;\">: ".$acctcreditspayment['savings_account_no']."</div></td>
            </tr>
            <tr>
        ...
        $filename = 'Kwitansi Angsuran Debet Tabungan.pdf';
