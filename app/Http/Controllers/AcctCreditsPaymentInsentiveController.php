...
class AcctCreditsPaymentInsentiveController extends Controller
{
    ...
    protected function pritCreditPayment($sesi)
    {
        ...
        if (!$bo) {
            $bo = ['incentive' => 0]; // Default jika $bo null
        }
        ...
        if (!empty($sesi['office_id'])) {
            $acctcreditspayment->where('acct_credits_account.office_id', $sesi['office_id']);
        }

        $acctcreditspayment = $acctcreditspayment->orderBy('acct_credits_account.credits_account_serial', 'ASC')->get();

        $groupedPayments = $acctcreditspayment->groupBy(function ($item, $key) {
        ...
        $header = "<table cellspacing=\"0\" cellpadding=\"1\" border=\"0\">
                <tr>
                    <td>
                        <div style=\"text-align: center; font-size:14px; padding-bottom: 10px;\">INSENTIF ANGSURAN TGL : &nbsp; " . date('d-m-Y', strtotime($sesi['start_date'])) . " - " . date('d-m-Y', strtotime($sesi['end_date'])) . "</div>
                    </td>
                </tr>
              </table>";
        ...
        if (!empty($sesi['office_id'])) {
            ...
            $header .= "<table cellspacing=\"0\" cellpadding=\"1\" border=\"0\" width=\"100%\">
                    <tr>
                        <td><div style=\"text-align: left; font-size:12px; font-weight:bold; padding-top: 5px;\">AO: " . $officeName . "</div></td>
                    </tr>
                </table>";
        } else {
            $header .= "<table cellspacing=\"0\" cellpadding=\"1\" border=\"0\" width=\"100%\">
                    <tr>
                        <td><div style=\"text-align: left; font-size:12px; font-weight:bold; padding-top: 5px;\">GLOBAL</div></td>
                    </tr>
                </table>";
        }
        ...
        $export = "<table cellspacing=\"0\" cellpadding=\"1\" border=\"0\" width=\"100%\">
                <thead>
                    <tr>";
        foreach ($thead as $th) {
            $export .= "<th style=\"border-bottom: 1px solid black; border-top: 1px solid black; text-align:center; font-size:10; padding: 6px 2px;\">$th</th>";
        }
        $export .= "</tr>
                </thead>
             </table>";
        ...
        foreach ($groupedPayments as $officeName => $payments) {
            $export .= "<table cellspacing=\"0\" cellpadding=\"1\" border=\"0\" width=\"100%\">
                    <tr>
                        <td colspan=\"6\" style=\"font-weight: bold; font-size: 10px; text-align: left; padding-top: 10px;\">$officeName</td>
                    </tr>";
            ...
            foreach ($payments as $val) {
                ...
                $export .= "<tr>
                        <td width=\"10%\"><div style=\"text-align: left; padding: 6px;\">" . $no . "</div></td>
                        <td width=\"20%\"><div style=\"text-align: left; padding: 6px;\">" . $val->account->credits_account_serial . "</div></td>
                        <td width=\"10%\"><div style=\"text-align: left; padding: 6px;\">" . $val->member->member_name . "</div></td>
                        <td width=\"20%\"><div style=\"text-align: right; padding: 6px;\">" . number_format($val->credits_principal_last_balance) . "</div></td>
                        <td width=\"20%\"><div style=\"text-align: right; padding: 6px;\">" . number_format($val['credits_payment_principal']) . "</div></td>
        ...
        $bottom = "<table>
                <tr>
                    <td colspan=\"3\"><div style=\"font-size:10px; text-align:left; font-style:italic; padding-top: 6px;\">Printed: " . date('d-m-Y H:i:s') . " " . Auth::user()->username . "</div></td>
                </tr>
             </table>";
    ...
    protected function exportCreditsPayment($sesi)
    {
        ...
        if (!empty($branch_id)) {
            $acctcreditspayment->where('branch_id', $branch_id);
        }
        if (!empty($sesi['office_id'])) {
            $acctcreditspayment->where('office_id', $sesi['office_id']);
        }
