...
use App\DataTables\AcctCreditsPaymentBranch\AcctCreditsPaymentBranchDataTable;
use App\DataTables\AcctCreditsPaymentBranch\AcctCreditsAccountDataTable;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;
use App\Helpers\Configuration;
use Elibyy\TCPDF\Facades\TCPDF;

class AcctCreditsPaymentBranchController extends Controller
{
    public function index(AcctCreditsPaymentBranchDataTable $dataTable)
    {
        session()->forget('data_creditspaymentbranchadd');
        $sessiondata = session()->get('filter_creditspaymentbranch');
    ...
    public function filter(Request $request){
        ...
        session()->put('filter_creditspaymentbranch', $sessiondata);

        return redirect('credits-payment-branch');
    }

    public function filterReset(){
        session()->forget('filter_creditspaymentbranch');
    ...
    {
        $sessiondata = session()->get('data_creditspaymentbranchadd');
        ...
        session()->put('data_creditspaymentbranchadd', $sessiondata);
    }
    ...
    public function add()
    {
        ...
        return view('content.AcctCreditsPaymentBranch.Add.index', compact('sessiondata', 'acctcreditsaccount', 'acctcreditspayment', 'angsuranke', 'angsuranpokok', 'angsuranbunga', 'credits_payment_fine_amount', 'credits_account_accumulated_fines', 'credits_payment_day_of_delay'));
    }

    public function modalAcctCreditsAccount(AcctCreditsAccountDataTable $dataTable)
    {
        return $dataTable->render('content.AcctCreditsPaymentBranch.Add.AcctCreditsAccountModal.index');
    }
    ...
    public function selectAcctCreditsAccount($credits_account_id)
    {
        ...
        return redirect('credits-payment-branch/add');
    }
    ...
    public function processAdd(Request $request)
    {
        ...
        $branch_asal_id 	= $request->credits_account_branch_id;
        ...
        try {
            ...
            $data_journal_cabang = array(
                ...
                'journal_voucher_title'			=> 'ANGSURAN TUNAI ANTAR CABANG'.$acctcashpayment_last['credits_name'].' '.$acctcashpayment_last['member_name'],
                'journal_voucher_description'	=> 'ANGSURAN TUNAI ANTAR CABANG'.$acctcashpayment_last['credits_name'].' '.$acctcashpayment_last['member_name'],
            ...
            $account_rak_id 			 = CoreBranch::select('account_rak_id')
            ->where('branch_id', $branch_asal_id)
            ->first()
            ->account_rak_id;

            $account_id_default_status 	= AcctAccount::select('account_default_status')
			->where('account_id', $account_rak_id)
			->where('data_state', 0)
            ->first()
            ->account_default_status;
            ...
            AcctJournalVoucher::create($data_journal_pusat);
            ...
            if($data['credits_id'] == $preferencecompany['deferred_margin_income']){
                $account_aka_id 			 = CoreBranch::select('account_aka_id')
                ->where('branch_id', auth()->user()->branch_id)
                ->first()
                ->account_aka_id;

                $account_id_default_status 	= AcctAccount::select('account_default_status')
                ->where('account_id', $account_aka_id)
                ->where('data_state', 0)
                ->first()
                ->account_default_status;
                ...
                $account_id_default_status 	= AcctAccount::select('account_default_status')
                ->where('account_id', $preferencecompany['account_deferred_margin_income'])
                ->where('data_state', 0)
                ->first()
                ->account_default_status;
                ...
                $data_debet =array(
                    ...
                    'journal_voucher_debit_amount'	=> $data['credits_payment_interest'],
                    'account_id_status'				=> 0,
                );
                AcctJournalVoucherItem::create($data_debet);
            } else {
            ...
            $income_account_id = AcctCredits::select('income_account_id')
            ->where('credits_id', $data['credits_id'])
            ->first()
            ->income_account_id;
            ...
            DB::commit();
            $message = array(
                'pesan' => 'Angsuran Tunai (Antar Cabang) berhasil ditambah',
                'alert' => 'success'
            );
        } catch (\Exception $e) {
            DB::rollback();
            $message = array(
                'pesan' => 'Angsuran Tunai (Antar Cabang) gagal ditambah',
                'alert' => 'error'
            );
        }
        
        return redirect('credits-payment-branch')->with($message);
    }
    ...
    public function printNote($credits_payment_id){
        ...
        $branch_from = CoreBranch::select('branch_name')
        ->where('branch_id', $acctcreditspayment['branch_id'])
        ->first()
        ->branch_name;

        $branch_to = AcctCreditsAccount::select('core_branch.branch_name')
        ->join('core_branch', 'core_branch.branch_id', 'acct_credits_account.branch_id')
        ->where('credits_account_id', $acctcreditspayment['credits_account_id'])
        ->first()
        ->branch_name;
        ...
        <table cellspacing=\"0\" cellpadding=\"1\" border=\"0\">
            <tr>
                <td width=\"25%\"></td>
                <td width=\"75%\"><div style=\"text-align: left; font-size:14px\">BUKTI SETORAN ANGSURAN ANTAR CABANG</div></td>
            </tr>
            <tr>
                <td width=\"25%\"></td>
        ...
        <table cellspacing=\"0\" cellpadding=\"1\" border=\"0\" width=\"100%\">
            ...
            <tr>
                <td width=\"20%\"><div style=\"text-align: left;\">Cabang Asal</div></td>
                <td width=\"50%\"><div style=\"text-align: left;\">: ".$branch_from."</div></td>
            </tr>
            <tr>
                <td width=\"20%\"><div style=\"text-align: left;\">Cabang Tujuan</div></td>
                <td width=\"50%\"><div style=\"text-align: left;\">: ".$branch_to."</div></td>
            </tr>
            <tr>
        ...
        $filename = 'Kwitansi Angsuran Tunai (Antar Cabang).pdf';
