...
class EMoneyTransactionController extends BasePPOBController
{
    ...
    public function infoPPOBTopUpEMoney()
    {
        $data = array();

        $data['url']        = 'https://ciptapro.com/cst_ciptasolutindo/ppob/TopupSales/info';
    ...
    public function getPPOBTopUpEMoneyProduct(Request $request)
    {
        ...
        $data_inquiry[0]        = array(
            'productCode'       => ($request->productCode ?? $request->ppob_product_category_code) ?? $request->ppob_product_code,
    ...
    public function paymentPPOBTopUpEMoney(Request $request)
    {
        ...
        $data_post                  = array(
            'ppob_product_code'             => ($request->productCode ?? $request->ppob_product_category_code) ?? $request->ppob_product_code,
            'ppob_agen_id'                    => ($request->member_id ?? $user->member_id),
        ...
        $ppobbalance    = CoreMember::join(
                'acct_savings_account',
                'acct_savings_account.savings_account_id',
                '=',
                'core_member.savings_account_id'
            )
            ->join(
                'acct_savings',
                'acct_savings.savings_id',
                '=',
        ...
        // return $response;
        $maintenance = 0;
        if ($maintenance == 1 || $this->isPPOBmaintenance()) {
            $response['error_paymentppobtopupemoney']         = TRUE;
            $response['error_msg_title']                     = "Transaksi Gagal";
            $response['ppob_transaction_remark']            = "Pemeliharaan Transaksi EMoney Sedang Berlangsung";
            return $response;
        }
                    ...
                    if ($inquiry_data['code'] == 200) {
                        ...
                        if ($datappob_transactions->save()) {
                            ...
                            if ($data_profitshares->save()) {
                                ...
                                Log::info(json_encode($data_jurnal));
                        ...
                        Log::error("PPOB Transaction Failed", $inquiry_data);
                        $datappob_transaction = array(
                        ...
                        $datappob_transactions = new PPOBTransaction();
                        $datappob_transactions->ppob_unique_code                 = $data_post['id_transaksi'] . "|" . $inquiry_data['code'] ?? '-';
                        $datappob_transactions->ppob_company_id                  = $ppob_company_id;
                        ...
                        $datappob_transactions->ppob_transaction_status          = $ppob_transaction_status;
                        $datappob_transactions->created_id                       = $data_post['member_id'];
                        $datappob_transactions->ppob_transaction_remark          = "Transaksi Gagal : {$data_post['id_transaksi']}";
                        $datappob_transactions->imei                             = $user['member_imei'];

                        if ($datappob_transactions->save()) {
                            $datappob_transaction_cipta = new PPOBTransactionCipta();
                            $datappob_transaction_cipta->ppob_unique_code                 = $data_post['id_transaksi'] . "|" . $inquiry_data['code'] ?? '-';
                            $datappob_transaction_cipta->ppob_company_id                  = $ppob_company_id;
                            ...
                            $datappob_transaction_cipta->ppob_transaction_status          = $ppob_transaction_status;
                            $datappob_transaction_cipta->created_id                       = $data_post['member_id'];
                            $datappob_transaction_cipta->ppob_transaction_remark          = "Transaksi Gagal : {$data_post['id_transaksi']}";
                            $datappob_transaction_cipta->imei                             = $user['member_imei'];
                            $datappob_transaction_cipta->save();
                        }
